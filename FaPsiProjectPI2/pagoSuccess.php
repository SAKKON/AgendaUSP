		...
		<div class="justify-content-center contcont card-body h-25">
			...
			<div class="Citas-main">
				<div>
					...
					// echo $dato['id'];
					//$query = "UPDATE `horarios_citas` SET `id_cita`='[value-1]',`id_paciente`='[value-2]', `paymentStatus`='[value-6]' WHERE 1"
					...
					//echo $userID;
					$query = "SELECT correo FROM pacientes WHERE id_paciente = '$userID'";
					$requestmail = mysqli_query($mysqli, $query);
					$mail = mysqli_fetch_assoc($requestmail);
					//echo $mail['correo'];
					$userMail = 0;
					$paymentStatus = $dato['data']['object']['payment_status'];
					$emailStripe = $dato['data']['object']['customer_details']['email'];
					
					if($mail['correo'] == $emailStripe){
					    echo '<h1>¡LISTO, TU CITA YA HA SIDO PAGADA CON ÉXITO!</h1>
                            </div>
                            			</div>
					...
					$queryPaymentStatus = "UPDATE horarios_citas SET paymentStatus = 'paid' WHERE paymentStatus != 'paid' AND id_paciente = '$userID' LIMIT 1;";
					mysqli_query($mysqli, $queryPaymentStatus);

					    
					}elseif($mail['correo'] != $emailStripe){
					    
					    echo '<h3>Error de procesamiento de pago: "Correo utilizado en pago no concuerda con el del usuario"</h3>
                                   <p>¡No te preocupes! Escribe en la sección de abajo el correo con el que se realizó el pago de tu cita para habilitarlo en el sistema: </p>
					               <form method="POST">
                                        <input type="text" id="correo" name="correo" placeholder="Correo" required><br></input>
                                        <button type="submit" class="font-weight-bold" id="submit_correo" name="submit_correo">Confirmar correo</button>
                                    </form>
                                    </div>
                            			</div>
                            		</div>';
					    if (isset($_POST["submit_correo"])) {
					        $confirmUserMail = $_POST['correo'];
					        $userMail = $confirmUserMail;
